...
async function renderVersionHistory() {
  await Excel.run(async (context) => {
    ...
    try {
      // Check if VersionHistory sheet exists first
      const sheets = context.workbook.worksheets
      sheets.load("items/name")
      await context.sync()

      const versionSheetExists = sheets.items.some((sheet) => sheet.name === "VersionHistory")

      if (!versionSheetExists) {
      ...
      const values = range.values.slice(1) // Skip header row
