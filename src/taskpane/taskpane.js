/** taskpane.js - FINAL FIXED version for dropdown behavior */
...
async function saveAndCommitVersion() {
  await Excel.run(async (context) => {
    ...
    await populateVersionDropdown(newVersion); // pass new version to select it
  });
}

async function populateVersionDropdown(selectedVersion = "") {
  await Excel.run(async (context) => {
